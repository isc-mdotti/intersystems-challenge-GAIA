...
The files total ~380 MB compressed. Each row is one source's complete light curve — arrays of flux measurements stored as quoted JSON-style strings like `"[1234.5,null,6789.0,...]"`. There are 48 columns per row; we need 3. Speed is the only metric.
...
Combined with `ThreadPoolExecutor(max_workers=20)` — one thread per file — this brought the time down to **~1.9 seconds**.
